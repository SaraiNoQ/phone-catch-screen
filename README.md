...
- **两套凭证。** 主令牌（config.json，明文，CLI 用）和设备令牌（配对产生，每台手机一个）。主令牌能管理访问权限，设备令牌不能。
- **设备令牌只存哈希。** `devices.json` 权限 `0600`，里面是 SHA-256，不是令牌本身。备份泄漏或被读走都不会直接交出访问权。
- **主令牌不进手机。** 查看页 HTML 不含任何令牌；`/api/status` 只在主令牌请求时返回含主令牌的 `viewerURL`。
- **配对码单次有效、5 分钟过期、每来源限 5 次错误。** 6 位数字的爆破窗口被压到 5 次尝试。
- **令牌比较是常量时间的**，不给出时序侧信道。
- **`server.host = 127.0.0.1` 是强制的**，不只是绑定地址。服务会检查连接来源，非本机来源直接拒绝 —— 就算绑定出了问题也拦得住。
- **没有 HTTPS。** 局域网内问题不大，但如果你在咖啡厅连公共 Wi-Fi，同网段的人可以嗅探到明文令牌。这种场景请用 Tailscale，别直接把端口暴露出去。
- **图片不缓存。** 所有响应都带 `Cache-Control: no-store`。
- **查看页不引用任何外部资源** —— 没有 CDN、没有统计、没有第三方 JS。一个能看到你屏幕的页面，不该同时加载别人的脚本。

一个需要注意的取舍：设备令牌存在浏览器 `localStorage` 里。这意味着任何能在那台手机上执行 JS 的东西都能读走它。对个人自用工具这是合理取舍；如果你要和别人共用一台设备，用完就点「解除配对」。
